// CISC 3320 - OS Project
// Spring 2015
//
// Trevaughn Daley - Group Leader
// Jason Tan
// Raymond Ye
// 
// This project is a simulation of an operating system. It works together with an existing
// program (SOS) that simulates a job stream. SOS feeds this program a series of jobs and job requests;
// like a real operating system, which then must field those requests and handle the life cycle of each of
...
deque<long> ioQueue;  // ioQueue. Contains jobnumbers
...
void Svc(long &a, long p[])
{
     ...
     switch(a){
          ...
          // used to check for invalid svc requests
          default:
               cout<<"ERROR: The value of 'a' is invalid!  Cannot do SVC request."<<endl;
     }
...
void swapper(long jobNum, bool makeSpace)
{
     if (!drumBusy && jobNum!=-1)
     {
          ...
          else if(makeSpace == true && jobTable >= 50)
          {
               for(long i=0; i< jobsList.size(); i++)
               {
                    //Looks for a matching job on jobsList as job on back of ioQueue
